...
async def on_message(message):
    ...
    if channel_id in game_session  and game_sessions[channel_id]['active']:
        try:
...
async def alllogs(ctx, channel: discord.TextChannel):
    ...
    alllogs_channels[str(ctx.guild.id)] = channel.id
    ...
    user_accounts = load_moderation_json('user_accounts.json')
    warnings = load_moderation_json('warnings.json')
    guild_sp = sp_data.get(str(ctx.guild.id), {})
    
    # Send info for each member
    for member in members:
        ...
        embed.add_field(name="Seasonal Points", value=str(sp), inline=True)
...
async def update(ctx):
    """Update user information from messages in the channel"""
    if not ctx.author.guild_permissions.administrator:
        await ctx.send("You need administrator permission to use this command.")
        return
    
    # Get the alllogs channel if set
    alllogs_channel_id = alllogs_channels.get(str(ctx.guild.id))
    
    # Read recent messages in current channel
    messages = []
    ...
    if not messages:
        await ctx.send("No messages with embeds found to update from.")
        return
    
    # Parse and update user data from embeds
    updated_count = 0
    user_accounts = load_moderation_json('user_accounts.json')
    
    for msg in messages:
        for embed in msg.embeds:
            # Try to extract user info from embed
            if embed.title and embed.title.startswith("User: "):
                # Extract IGN from embed fields
                for field in embed.fields:
                    if field.name == "IGN" and field.value != "Not linked":
                        # Find mentioned user
                        if msg.mentions:
                            user = msg.mentions[0]
                            ...
                            user_accounts[key]['ign'] = field.value
                            user_accounts[key]['user_id'] = user.id
                            user_accounts[key]['guild_id'] = ctx.guild.id
                            
                            updated_count += 1
                            break
    
    save_moderation_json('user_accounts.json', user_accounts)
    await ctx.send(f"✅ Updated {updated_count} user account(s) from channel messages!")
