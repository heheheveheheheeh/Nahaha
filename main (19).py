...
async def add_sp(guild_id, user_id, sp):
    ...
    if guild:
        await auto_update_alllogs(guild)
    ...
    if len(tournament.results) == len(current_round):
        if len(tournament.results) == 1:
            ...
            if hasattr(winner_data, 'id') and not isinstance(winner_data, FakePlayer):
                await add_sp(ctx.guild.id, winner_data.id, 3)
            ...
            if len(all_eliminated) >= 1:
                ...
                if hasattr(player, 'id') and not isinstance(player, FakePlayer):
                    await add_sp(ctx.guild.id, player.id, 2)
            ...
            if len(all_eliminated) >= 2:
                ...
                if hasattr(player, 'id') and not isinstance(player, FakePlayer):
                    await add_sp(ctx.guild.id, player.id, 1)
...
async def sp_add(ctx, amount: int, member: discord.Member):
    ...
    await add_sp(ctx.guild.id, member.id, amount)
...
async def unban(ctx, *, member_identifier):
    ...
    if member_identifier.startswith('<@') and member_identifier.endswith('>'):
        # It's a mention
        user_id = int(member_identifier.strip('<@!>'))
    elif member_identifier.isdigit():
        # It's a direct ID
        user_id = int(member_identifier)
    
    for ban_entry in banned_users:
        user = ban_entry.user
        # Check by ID, username, or full tag
        if (user_id and user.id == user_id) or user.name.lower() == member_identifier.lower() or str(user) == member_identifier:
            try:
...
class AccountLinkModal(discord.ui.Modal, title="Link Your Account"):
    ...
    async def on_submit(self, interaction: discord.Interaction):
        ...
        # Auto-update alllogs
        await auto_update_alllogs(interaction.guild)
...
class TicketViewNew(discord.ui.View):
    def __init__(self, types_with_emojis):
        super().__init__(timeout=None)
        self.types_with_emojis = types_with_emojis
        
        for emoji, ticket_type in types_with_emojis[:25]:  # Discord limit
    ...
    async def create_ticket_callback(self, interaction: discord.Interaction):
        ...
        ticket_type = interaction.data['custom_id'].replace('ticket_', '').replace('_', ' ')
    ...
    async def create_ticket(self, interaction: discord.Interaction, ticket_type):
        ...
        # Create ticket channel with simplified name (no emoji)
...
class TicketView(discord.ui.View):
    def __init__(self, ticket_types):
        ...
        # Add buttons for each ticket type with server emojis
        emojis = ['🎫', '🛠️', '💡', '❓', '🚨', '📞', '💰', '🎮', '🏆', '⚡', '🔧', '📋', '🎯', '🎪', '🎨', '🎭', '🎪', '🎺', '🎻', '🎸', '🎹', '🥁', '🎤', '🎧', '🎬']
        
        for i, ticket_type in enumerate(ticket_types[:25]):  # Discord limit
            emoji = emojis[i % len(emojis)]  # Use server emoji or fallback to unicode
            button = discord.ui.Button(
                label=ticket_type,
                emoji=emoji,
                style=discord.ButtonStyle.secondary,
    ...
    async def create_ticket(self, interaction: discord.Interaction, ticket_type):
        ...
        channel_name = f"ticket-{ticket_type.lower().replace(' ', '-')}-{user.name}"
...
async def ticket(ctx, *, ticket_types):
    ...
    # Parse ticket types and emojis from input
    # Format: emoji1 type1, emoji2 type2 OR type1,type2 (without emojis)
    items = [t.strip() for t in ticket_types.split(',')]
    
    types_with_emojis = []
    for item in items:
        parts = item.split(None, 1)  # Split by first space
        if len(parts) == 2 and len(parts[0]) <= 4:  # Likely emoji + type
            emoji = parts[0]
            ticket_type = parts[1]
            types_with_emojis.append((emoji, ticket_type))
        else:
            types_with_emojis.append((None, item))
    
    if len(types_with_emojis) > 25:  # Discord has a limit of 25 buttons per view
    ...
    view = TicketViewNew(types_with_emojis)
    await ctx.send(embed=embed, view=view)

@bot.command()
async def game(ctx, number_range: str = "1-20"):
    """Start a number guessing game"""
    # Restricted access - staff but not admin only
    if not await is_staff(ctx):
        await ctx.send("You don't have permission to use this command. This game is for staff members only.")
        return
...
@bot.command()
async def embed(ctx, color: str, *, message: str):
    """Send a message in a colored embed"""
    if not ctx.author.guild_permissions.administrator:
        await ctx.send("You need administrator permission to use this command.")
        return
    
    # Parse color - support hex colors and common color names
    color_map = {
        'red': 0xff0000,
        'blue': 0x0000ff,
        'green': 0x00ff00,
    ...
    try:
        ...
        if color.startswith('#'):
            ...
            # Try direct hex without #
            embed_color = int(color, 16)
    except:
        await ctx.send("Invalid color! Use hex (#ff0000) or color name (red, blue, green, etc.)")
        return
...
@bot.command()
async def message(ctx, time_interval: str, *, message: str):
    ...
    # Parse time interval
    duration = parse_time(time_interval)
    if not duration:
        await ctx.send("Invalid time format! Use format like: 1m, 5m, 1h, etc.")
        return
    
    # Create unique key for this scheduled message
    schedule_key = f"{ctx.guild.id}_{ctx.channel.id}_{len(scheduled_messages)}"
    
    # Create the scheduled message task
    async def send_scheduled():
        while schedule_key in scheduled_messages:
            await asyncio.sleep(duration.total_seconds())
            if schedule_key in scheduled_messages:
                await ctx.channel.send(message)
    
    # Store and start the task
    scheduled_messages[schedule_key] = asyncio.create_task(send_scheduled())
    
    await ctx.send(f"✅ Scheduled message will be sent every {time_interval} in this channel!")

@bot.command()
async def stopmessage(ctx):
    """Stop all scheduled messages in this channel"""
    if not ctx.author.guild_permissions.administrator:
        await ctx.send("You need administrator permission to use this command.")
        return
    
    # Find and cancel all scheduled messages for this channel
    to_remove = []
    for key in scheduled_messages:
        if key.startswith(f"{ctx.guild.id}_{ctx.channel.id}_"):
    ...
    if to_remove:
        await ctx.send(f"✅ Stopped {len(to_remove)} scheduled message(s) in this channel.")
    else:
        await ctx.send("No scheduled messages found in this channel.")

# Store alllogs channel and message IDs
alllogs_channels = {}
alllogs_messages = {}

async def auto_update_alllogs(guild):
    """Automatically update alllogs display when data changes"""
    guild_str = str(guild.id)
    
    # Check if alllogs is set up
    if guild_str not in alllogs_channels:
        return
    
    channel = guild.get_channel(alllogs_channels[guild_str])
    if not channel:
        return
    
    # Delete old messages
    if guild_str in alllogs_messages:
        for msg_id in alllogs_messages[guild_str]:
            try:
    ...
    user_accounts = load_moderation_json('user_accounts.json')
    warnings = load_moderation_json('warnings.json')
    guild_sp = sp_data.get(guild_str, {})
    
    # Build user info text
    user_lines = []
    for member in members:
        key = f"{guild.id}_{member.id}"
        account_data = user_accounts.get(key, {})
        ign = account_data.get('ign', 'Not linked')
        sp = guild_sp.get(str(member.id), 0)
        user_warnings = [w for w in warnings if w.get('user_id') == member.id and w.get('guild_id') == guild.id]
        warning_count = len(user_warnings)
        
        user_lines.append(f"{member.mention} | IGN: {ign} | SP: {sp} | Warnings: {warning_count}")
    ...
    # Send new embeds and store message IDs
    alllogs_messages[guild_str] = []
    for i, chunk in enumerate(chunks):
        embed = discord.Embed(
            title=f"📊 All Users Information {f'(Part {i+1}/{len(chunks)})' if len(chunks) > 1 else ''}",
            description=chunk,
            color=0x0099ff
        )
        msg = await channel.send(embed=embed)
        alllogs_messages[guild_str].append(msg.id)

@bot.command()
async def alllogs(ctx, channel: discord.TextChannel):
    """Set up a channel to display all user information"""
    if not ctx.author.guild_permissions.administrator:
        await ctx.send("You need administrator permission to use this command.")
        return
    
    # Store the alllogs channel
    guild_str = str(ctx.guild.id)
    alllogs_channels[guild_str] = channel.id
    ...
    for member in members:
        ...
        ign = account_data.get('ign', 'Not linked')
        sp = guild_sp.get(str(member.id), 0)
        user_warnings = [w for w in warnings if w.get('user_id') == member.id and w.get('guild_id') == ctx.guild.id]
        warning_count = len(user_warnings)
        
        user_lines.append(f"{member.mention} | IGN: {ign} | SP: {sp} | Warnings: {warning_count}")
    
    # Split into chunks if needed (Discord embed description limit is 4096 characters)
    chunk_size = 4000
    chunks = []
    current_chunk = []
    current_length = 0
    ...
    await ctx.send(f"✅ All user logs sent to {channel.mention}!")

@bot.command()
async def update(ctx):
    """Update alllogs display with current data"""
    if not ctx.author.guild_permissions.administrator:
        await ctx.send("You need administrator permission to use this command.")
        return
    ...
    # Get the alllogs channel
    alllogs_channel_id = alllogs_channels.get(guild_str)
    if not alllogs_channel_id:
        await ctx.send("No alllogs channel set! Use !alllogs #channel first.")
        return
    
    channel = ctx.guild.get_channel(alllogs_channel_id)
    if not channel:
        await ctx.send("Alllogs channel not found!")
        return
    ...
    await ctx.send(f"✅ Updated all user logs in {channel.mention}!")
